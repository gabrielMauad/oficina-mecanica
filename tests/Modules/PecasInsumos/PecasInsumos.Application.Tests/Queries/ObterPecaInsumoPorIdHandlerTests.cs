...
public class ObterPecaInsumoPorIdHandlerTests
{
    ...
    public ObterPecaInsumoPorIdHandlerTests()
    {
        ...
        _handler = new ObterPecaInsumoPorIdHandler(_repositoryMock.Object);
    }
    ...
    {
        // Arrange
        var query = new ObterPecaInsumoPorIdQuery(
            Guid.NewGuid()
        );
        ...
        Assert.Equal(pecaInsumo.Id.Value, result.Value.Id);
        Assert.Equal("Filtro de Óleo", result.Value.Nome);
        Assert.Equal("Descrição inicial", result.Value.Descricao);
        Assert.Equal(49.90m, result.Value.PrecoUnitario);
        Assert.Equal(5, result.Value.QuantidadeEmEstoque);
        Assert.Equal(UnidadeDeMedida.Unidade.ToString(), result.Value.UnidadeDeMedida);
        Assert.True(result.Value.Ativo);
        Assert.Equal(pecaInsumo.CadastradoEm, result.Value.CadastradoEm);
        Assert.Equal(pecaInsumo.AtualizadoEm, result.Value.AtualizadoEm);
    }
